...
export class FactorComputeEngine {
  ...
  public static evaluate(ast: FactorAST, bar: YahooBar): number {
    ...
    if (ast.type === "operator") {
      const leftVal = ast.left
        ? FactorComputeEngine.evaluate(ast.left, bar)
        : 0;
      const rightVal = ast.right
        ? FactorComputeEngine.evaluate(ast.right, bar)
        : 0;
  ...
  public static computeSeries(ast: any, bars: YahooBar[]): number[] {
    ...
    return bars.map((bar) => FactorComputeEngine.evaluate(typedAST, bar));
  }
}
