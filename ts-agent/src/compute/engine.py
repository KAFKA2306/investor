...
def evaluate_ast(ast: Dict[str, Any], df: pd.DataFrame) -> pd.Series:
    ...
    if node_type == "operator":
        ...
        if left_ast is None: return pd.Series(0, index=df.index)
        left = evaluate_ast(left_ast, df)
        
        if op == "SMA":
            right_ast = ast.get("right", {})
            window = right_ast.get("value", 5) if right_ast.get("type") == "constant" else 5
            return left.rolling(window=int(window), min_periods=1).mean()
            
        if op == "LAG":
            right_ast = ast.get("right", {})
            periods = right_ast.get("value", 1) if right_ast.get("type") == "constant" else 1
            return left.shift(int(periods))
            
        right_ast = ast.get("right")
        if right_ast is None: return left
        right = evaluate_ast(right_ast, df)
        
        if op == "ADD": return left + right
        if op == "SUB": return left - right
        if op == "MUL": return left * right
        if op == "DIV": return left / right.replace(0, np.nan)
