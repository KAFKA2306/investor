...
SUPPORTED_OPS = {
    ...
    "CS_ZScore": lambda s: (s - s.mean()) / s.std(),
}
...
def eval_formula(formula: str, df: pd.DataFrame) -> pd.Series:
    def resolve(expr: str) -> pd.Series:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to eval arithmetic expression '{expr}': {e}"
            ) from e
...
def evaluate_factors(request: dict) -> dict:
    ...
    for factor in factors:
        ...
        try:
            ...
            results.append(
                {"id": fid, "status": "success", "scores": scores_by_symbol}
            )
        except Exception as e:
            results.append({"id": fid, "status": "error", "message": str(e)})
