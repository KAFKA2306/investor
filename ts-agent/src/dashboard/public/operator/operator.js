(() => {
  ...
  function hypothesisRecords(payload) {
    const candidates = [
      payload?.hypotheses,
      payload?.evidence?.hypotheses,
      payload?.research?.hypotheses,
      payload?.registry?.hypotheses,
    ];
  ...
  function runRecords(payload) {
    const candidates = [
      ...
      payload?.research?.runs,
      payload?.executions,
      payload?.operational?.runs,
    ];
    const found = candidates.find(Array.isArray);
    return found || [];
  }
  ...
  function missingConditions(record) {
    ...
    if (explicit.length)
      return explicit.map((value) =>
        typeof value === "object"
          ? text(value, "message", "name", "reason", "status")
          : String(value),
      );
    return gateResults(record)
      .filter(
        (gate) =>
          !boolean(pick(gate, "passed", "eligible", "status", "result")),
      )
      .map((gate) => text(gate, "message", "name", "gate", "reason"));
  }

  function recordState(record) {
    return (
      normalize(
        pick(record, "state", "status", "decision", "lifecycle_state", "stage"),
      ) || "unknown"
    );
  }
  ...
  function queueFor(record) {
    ...
    if (/(reject|retir|stop|fail|invalid|killed)/.test(current))
      return "rejected";
    if (
      boolean(eligible) ||
      /(review|candidate|eligible|promotable|technical_ready)/.test(current)
    )
      return "review";
    if (
      /(oos|frozen|awaiting_evaluation|registered|backtest)/.test(current) ||
      oos === false ||
      normalize(oos).includes("pending")
    )
      return "oos";
    if (
      missing.length ||
      /(attention|stale|partial|error|blocked|unknown)/.test(current)
    )
      return "attention";
    return "attention";
  }
  ...
  function freshnessFor(record) {
    const explicit = normalize(
      pick(record, "freshness", "data_freshness", "evidence_freshness"),
    );
  ...
  function nextGate(record) {
    ...
    if (explicit)
      return typeof explicit === "object"
        ? text(explicit, "name", "message", "status")
        : String(explicit);
    const failed = gateResults(record).find(
      (gate) => !boolean(pick(gate, "passed", "eligible", "status", "result")),
    );
  ...
  function evidenceLinks(record) {
    ...
    return raw.map((entry, index) => {
      if (typeof entry === "string")
        return { label: `証拠 ${index + 1}`, href: entry };
      return {
        label: text(entry, "label", "title", "name", "type", "path"),
        href: pick(entry, "url", "href", "path"),
      };
    });
  }
  ...
  function falsifiers(record) {
    return asArray(
      ...
      typeof value === "object"
        ? text(value, "message", "name", "condition")
        : String(value),
    );
  }
  ...
  function displayName(record, index) {
    return text(
      ...
      ? `仮説 ${index + 1}`
      : text(record, "hypothesis_name", "name", "title", "hypothesis_id", "id");
  }
  ...
  function searchable(record) {
    return normalize(
      [
        ...
        record.risk,
        record.command,
        record.evidence.map((item) => item.label).join(" "),
      ].join(" "),
    );
  }
  ...
  function filteredRecords() {
    ...
    return state.records.filter((record) => {
      ...
      const matchesState =
        state.stateFilter === "all" || record.state === state.stateFilter;
      const matchesQueue =
        state.queueFilter === "all" || record.queue === state.queueFilter;
      const matchesFreshness =
        state.freshness === "all" || record.freshness === state.freshness;
  ...
  function renderCard(record) {
    ...
    card.querySelector('[data-field="next-gate"]').textContent =
      record.nextGate;
    card.querySelector('[data-field="missing"]').textContent = record.missing
      .length
      ? record.missing.join(" / ")
      : "なし";
    card.querySelector('[data-field="last-run"]').textContent = record.lastRun;
    card.querySelector('[data-field="data-as-of"]').textContent =
      record.dataAsOf;
    ...
    card.querySelector('[data-field="evidence"]').innerHTML = record.evidence
      .length
      ? record.evidence
          .map((item) =>
            item.href
          ...
          .join("")
      : "<li>証拠リンクなし</li>";
    card.querySelector('[data-field="falsifiers"]').innerHTML = record
      .falsifiers.length
      ? record.falsifiers.map((item) => `<li>${escapeHtml(item)}</li>`).join("")
      : "<li>未登録</li>";
    return card;
  }
  ...
  function renderQueues() {
    ...
    queues.forEach((queue) => {
      ...
      if (!queueRecords.length)
        grid.innerHTML = '<div class="empty">該当する仮説はありません。</div>';
      count.textContent = `${queueRecords.length}件`;
      section.hidden =
        state.queueFilter !== "all" && state.queueFilter !== queue;
      byId(`metric-${queue}`).textContent = state.records.filter(
        (record) => record.queue === queue,
      ).length;
    });
  ...
  function populateStateOptions() {
    ...
    [...new Set(state.records.map((record) => record.state))]
      .sort()
      .forEach((value) => {
  ...
  function renderRisks(payload) {
    ...
    const defaults = [
      {
        title: "実績と研究を分離",
        description: "BacktestやPaperをLive実績として表示しません。",
      },
      {
        title: "費用と最大DDを先に確認",
        description:
          "収益率より期間・比較対象・費用・最大ドローダウンを優先します。",
      },
      {
        title: "古い証拠を正常扱いしない",
        description: "データ時点や実行IDがない仮説は要対応へ分類します。",
      },
    ];
    const risks = raw.length
      ? raw.map((item, index) =>
          typeof item === "object"
            ? {
                title:
                  text(item, "title", "name", "type") === "—"
                    ? `リスク ${index + 1}`
                    : text(item, "title", "name", "type"),
                description: text(
                  item,
            ...
            : { title: `リスク ${index + 1}`, description: String(item) },
        )
      : defaults;
    byId("risk-grid").innerHTML = risks
      .slice(0, 9)
      .map(
        (item) =>
          `<article class="risk-card"><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.description)}</p></article>`,
      )
      .join("");
  }
  ...
  function bindControls() {
    ...
    byId("queue-filter").addEventListener("change", (event) => {
      state.queueFilter = event.target.value;
      renderQueues();
    });
    byId("freshness-filter").addEventListener("change", (event) => {
  ...
  async function load() {
    ...
    try {
      ...
      byId("system-state").textContent = text(
        payload,
        "system_state",
        "evidence_status",
        "status",
      );
      byId("as-of").textContent =
        `基準日時 ${text(payload, "as_of", "generated_at", "updated_at")}`;
      byId("system-message").textContent = state.records.length
        ? `${state.records.length}件の仮説を評価しました。`
        : "仮説レジストリに公開可能な項目がありません。";
      ...
      byId("system-message").textContent =
        `Evidence APIを読み込めません: ${error.message}`;
      state.records = [];
      bindControls();
      renderQueues();
      renderRisks({});
