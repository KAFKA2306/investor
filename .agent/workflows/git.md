...
お掃除が完璧に終わったら、`git status` で変更を確認し、機能ごとに **「アトミック（最小単位）」に分割して** ステージング (`git add`) しようねっ！
- **指示 (Agent Prompt)**:
  - 変更が複数の機能やファイルにまたがる場合は、一気に `git add .` するんじゃなくて、**意味のある単位で分割してコミットすること。**
  - 分割の目安は、「ドキュメント修正」「プロバイダー実装」「実験ロジック」「システム基盤」など、機能の役割（Responsibility）ごとだよっ ✨
  - Commit メッセージは `feat:`, `fix:`, `docs:`, `refactor:`, `chore:` などのプレフィックスを必ずつけること。

### 2.5. ぐたいてきな「きのうせい」の明記っ ✨ (Commit Specificity)
コミットメッセージや変更内容には、必ず **具体性と機能性** を持たせようねっ！
- **指示 (Agent Prompt)**:
  - 「なにを (What)」変えたかだけじゃなくて、「どんな機能 (Functionality)」が追加・改善されたかを意識すること。
  - コミット履歴を読んだだけで、プロジェクトがどう進化したか「具体的（Concrete）」にわかるのが理想だよっ ✨
  - かわいい日本語を使いつつも、技術的な意図はビシッと伝えること！💖
