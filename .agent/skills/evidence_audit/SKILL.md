...
- **是正アクション**: `simulator.ts` または `QuantMetrics` を使用し、観測されたリターン系列から動的に算出するよう強制する。
...
2. **証拠ソースの明示**: 全ての `StandardOutcome` に `evidenceSource` プロパティ（`QUANT_BACKTEST` | `LINGUISTIC_ONLY`）を付与し、フロントエンドに透過的に伝えること。
